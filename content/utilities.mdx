...
```python
zwylib.copy_to_clipboard(bulletin_helper: Optional[_BulletinHelper], text_to_copy: str) -> None
```
...
- `bulletin_helper` (`Optional[_BulletinHelper]`): Instance of a bulletin helper to show the success message. If `None`, no bulletin is shown.
...
- `current_try` (`bool`, default `False`): Must not be set manually. Current plugin download try.
